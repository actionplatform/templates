...
├── schemas/       Data response contracts
├── services/      business rules; raise DomainError; no Sinatra
├── repositories/  data access; no rules, no HTTP
└── core/          errors.rb (DomainError) and base_api.rb (→ {"detail": {code, message, field}})
```

Request → api → service → repository. Never put a rule in a route; never require Sinatra in a service.
